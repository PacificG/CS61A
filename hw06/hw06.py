...
class Coin:
    ...
    def worth(self):
        ...
        return self.cents + (0 if (diff) <= 50 else diff - 50)
...
def is_bst(t):
    ...
    def helper(t):
        if t.is_leaf():
            ...
            l, r = t.branches
            if t.label >= max_bst(l) and t.label < min_bst(r):
                if t.label < r.label and t.label >= l.label:
                    return helper(l) and helper(r)
                else:
                    return False
            else:
                return False 
