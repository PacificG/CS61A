...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        ls = subseqs(s[1:])
        return ls + insert_into_all(s[0], ls)
...
def make_glookup(class_assignments):
    ...
    "*** YOUR CODE HERE ***"
    dic = {}
    def current_grade(assign, grade):
        nonlocal dic
        dic[assign] = grade
        return sum(dic.values())/sum([class_assignments[key] for key in dic])
...
def make_advanced_counter_maker():
    ...
    def make_counter():
        icount = 0
        def inv_counter(access):
            nonlocal count
            nonlocal icount
            if access == 'count':
                icount += 1
                return icount
            elif access == 'global-count':
                count += 1
                return count
            elif access == 'global-reset':
                count = 0
            elif access == 'reset':
                icount = 0
